...
export async function load({ params }) {
 ...
 const bio = people.find(d => d.id === id);
	...
	const cv = 
		...
		(id === '167' && (await import('$lib/data/cv/167.json')).default) ||
		(id === '168' && (await import('$lib/data/cv/168.json')).default)
 ...
 if (!bio) {
	...
	const description_en = `${title_en} is a ${bio.position}, ${bio.institution_en}`;
	const description_si = `${title_si} je ${getPositionLabel(bio.position, bio.gender, "si")}, ${bio.institution_si}`
